...
#RNA-seq data from The Cancer Genome Atlas Program (TCGA). COAD - Colon Adenocarcinoma
query <- GDCquery(
  project = "TCGA-COAD",data.category = "Gene expression",data.type = "Gene expression quantification",
...
coad_raw_data <- GDCprepare(query = query)
coad_raw_data@assays@data@listData[["raw_count"]] <- round(coad_raw_data@assays@data@listData[["raw_count"]])
dds <- DESeqDataSet(coad_raw_data, design = ~ sample_type_id)
...
structured_coad <- t(rbind(vsd@colData@listData[["sample_type_id"]],vsd@assays@data@listData[["raw_count"]]))
write.csv(structured_coad,"coad_structured_data.csv")

#create single-case model testing csv files
#Sample 
sample_10 <- t(structured_kirc[1,])
...
coad_train_labels <- cbind(image_names,sample_type_id)
write.csv(coad_train_labels,"coad_train_labels.csv")