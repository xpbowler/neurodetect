...
#RNA-seq data from The Cancer Genome Atlas Program (TCGA). KIRC - Kidney Renal Clear Cell Carcinoma
#"01"="Primary Tumor" || "11"="Solid Tissue Normal"
# or in dds
#"1"="Primary Tumor" || "2"="Solid Tissue Normal"
query <- GDCquery(project = "TCGA-KIRC",data.category = "Gene expression",data.type = "Gene expression quantification",
...
raw_data <- GDCprepare(query = query)
raw_data@assays@data@listData[["raw_count"]] <- round(raw_data@assays@data@listData[["raw_count"]])
dds <- DESeqDataSet(raw_data, design = ~ sample_type_id)

#plot PCA, heatmap, and differential gene expression plot 
...
structured_... <- t(rbind(vsd@colData@listData[["sample_type_id"]],vsd@assays@data@listData[["raw_count"]]))
write.csv(structured_...,"vsd_structured_data.csv")

#... is a placeholder for the cancer type code, such as BRCA for Breast Cancer 
#create single-case model testing csv files
sample_10 <- t(structured_...[1,])
...
..._train_labels <- cbind(image_names,sample_type_id)
write.csv(..._train_labels,"..._train_labels.csv")